...
function App() {
  ...
  const handleCheckoutComplete = (order: Order) => {
    ...
    setView({ type: 'order-confirmation', order });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <div className="min-h-screen bg-[#F5F2EB] font-sans text-[#2C2A26] selection:bg-[#D6D1C7] selection:text-[#2C2A26]">
      {view.type !== 'checkout' && view.type !== 'order-confirmation' && (
        <Navbar 
      ...
      {view.type !== 'checkout' && view.type !== 'order-confirmation' && <Footer onLinkClick={handleNavClick} />}
